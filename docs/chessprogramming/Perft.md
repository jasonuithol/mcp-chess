...
To generate legal moves, some programs have to make moves first, call a function to check if the position in-check and then undo those moves. That makes the above Perft function make and undo moves twice for all moves. The code below can avoid that problem and run much faster:
...
Perft can receive another speed boost by [hashing](/Hash_Table "Hash Table") node counts, with a small chance for inaccurate results. Sometimes this is used as a sanity check to make sure the hash table and keys are working correctly. Hashing may speed up from 1.5 to 4 times[[1]](#cite_note-1).

# Divide

The Divide command is often implemented as a variation of Perft, listing all moves and for each move, the Perft of the decremented depth. However, some programs already give "divided" output for Perft. Below is the output of [Stockfish](/Stockfish "Stockfish") when computing perft 5 for the start position:
...
Perft is mostly for debugging purposes. It works mainly with functions: move generators, make move, and unmake move. They are all very basic and vital for chess engines. By comparing Perft results, developers can determine if those functions are working correctly or not. If they are incorrect, they can narrow down the search quickly by comparing branches. Then, they can call Perft for the wrong branches with lower depth and repeat the process until they find direct positions that yield the wrong result.

Other purposes:

- give a glance at how good/bad their generators/make/unmake functions are, compared with the speed of other engines
- calculate branch factors
- a factor to estimate the complexity of chess variants, by comparing branch factors or Perft results at a given depth for their starting positions

# Known issues

Perft algorithm considers only legal moves but ignores some [draw rules](/Draw "Draw"). Before reaching the limit depth, it expands a position if it has some legal moves and stops if it does not. That means Perft stops at being mated and [draw-by-stalemate](/Stalemate "Stalemate") positions. However, since the algorithm doesn’t mention, Perft doesn’t stop at other draw positions, including 1) [fifty-move](/Fifty-move_Rule "Fifty-move Rule") 2) [repetitions](/Repetitions "Repetitions"), and 3) [insufficient material](/Material#InsufficientMaterial "Material"). Consequently, the Perft search trees are not the Chess [search trees](/Search_Tree "Search Tree") even though they are quite close. The reason is from the historical when some early chess programs used Perft in that way, and then many others adopted it. The Perft's original form has been widely accepted because of its simple, fast and well-served debugging purposes.

# History

Supposably, perft was first implemented within the [Cobol](/index.php?title=Cobol&action=edit&redlink=1 "Cobol (page does not exist)") program [RSCE-1](/index.php?title=RSCE-1&action=edit&redlink=1 "RSCE-1 (page does not exist)") by [R.C. Smith](/Rolf_C._Smith#RCSmith "Rolf C. Smith"), submitted to the [USCF](https://en.wikipedia.org/wiki/United_States_Chess_Federation) for evaluation, and subject of an [1978](/Timeline#1978 "Timeline") [Computerworld](/Computerworld "Computerworld") article [[2]](#cite_note-2) . RSCE-1's purpose was not to play chess games, but to position analysis, to find forced [mates](/Checkmate "Checkmate"), and to perform a moving path enumeration of up to three [plies](/Ply "Ply"), with the [perft(3) result](/Perft_Results "Perft Results") of 8,902 from the [initial position](/Initial_Position "Initial Position") already mentioned [[3]](#cite_note-3). [Ken Thompson](/Ken_Thompson "Ken Thompson") may have calculated perft(3) and perft(4) earlier than this date with [Belle](/Belle "Belle"). [Steven Edwards](/Steven_Edwards "Steven Edwards") suggested the move path enumeration in 1995 as implemented in [Spector](/Spector "Spector") [[4]](#cite_note-4) and has since been actively involved in Perft computations, while the term "Perft" was likely coined by a [Crafty](/Crafty "Crafty") command, despite its initial implementation was not conformed to the above definition [[5]](#cite_note-5).

In **December 2003**, [Albert Bertilsson](/Albert_Bertilsson "Albert Bertilsson") started a distributed project [[6]](#cite_note-6) to calculate perft(11) of the [initial position](/Initial_Position "Initial Position"), taking over a week to calculate [[7]](#cite_note-7) . Exact Perft numbers have been computed and verified up to a depth of 13 by Edwards and are now available in the [On-Line Encyclopedia of Integer Sequences](https://en.wikipedia.org/wiki/On-Line_Encyclopedia_of_Integer_Sequences) [[8]](#cite_note-8), and are given under [Initial Position Summary](/Initial_Position_Summary "Initial Position Summary"). A so far unverified claim for perft(**14**) of 61,885,021,521,585,529,237 was given by [Peter Österlund](/Peter_%C3%96sterlund "Peter Österlund") in **April 2013** [[9]](#cite_note-9), while [Daniel Shawul](/Daniel_Shawul "Daniel Shawul") proposed Perft estimation applying [Monte carlo methods](/Monte-Carlo_Tree_Search "Monte-Carlo Tree Search") [[10]](#cite_note-10) [[11]](#cite_note-11).
In **August 2017**, [Ankan Banerjee](/Ankan_Banerjee "Ankan Banerjee"), who already confirmed Peter Österlund's perft(**14**) in September 2016 [[12]](#cite_note-12), computed perft(**15**) of 2,015,099,950,053,364,471,960 with his [GPU](/GPU "GPU") perft program [[13]](#cite_note-13), running it several days two times with different [zobrist keys](/Zobrist_Hashing "Zobrist Hashing") on a cluster of [Nvidia DGX-1](https://en.wikipedia.org/wiki/Nvidia_DGX-1) server systems [[14]](#cite_note-14). His program starts exploring the tree in [depth first](/Depth-First "Depth-First") manner on the CPU. When a certain depth is reached a GPU function (kernel) is launched to compute perft of the subtree in [breadth first](/Best-First "Best-First") manner [[15]](#cite_note-15). Ankan Banerjee dedicated his computations in honor to [Steven Edwards](/Steven_Edwards "Steven Edwards") - whose tireless efforts for verifying perft(14) encouraged him to verify perft(14) and take up the challenge to compute perft(15) [[16]](#cite_note-16).

# Quotes

by [Robert Hyatt](/Robert_Hyatt "Robert Hyatt") in a forum post, June 12, 2020 [[17]](#cite_note-17) :

```
I believe I was the first to use this. Back in the 80s. We rewrote the move generator in Cray Blitz in assembly language. It was a pain to debug. I decided on the "perft" approach solely to test/debug the move generator. We'd run two versions, one FORTRAN, one assembly, and we tested and debugged until they matched.
I carried this over into Crafty as early versions went through several different approaches on move generation. Starting with the Slate/Atkin approach, then rotated bitboards (which took some time to debug), and the magic. It was really intended solely for that purpose. Then several started to use it as a benchmark for speed. I never followed that path since move generation is a very small part of the overall CPU time burned.
...
- [Daniel S. Abdi](/Daniel_Shawul "Daniel Shawul") (**2013**). *Monte carlo methods for estimating game tree size*. [[18]](#cite_note-18) » [Monte-Carlo Tree Search](/Monte-Carlo_Tree_Search "Monte-Carlo Tree Search")
...
- [Perft(3) from 1978, with a twist!](http://www.talkchess.com/forum/viewtopic.php?t=41373) by [Steven Edwards](/Steven_Edwards "Steven Edwards"), [CCC](/CCC "CCC"), December 08, 2011 [[19]](#cite_note-19)
...
- [Fast perft on GPU (upto 20 Billion nps w/o hashing)](http://www.talkchess.com/forum/viewtopic.php?t=48387) by [Ankan Banerjee](/Ankan_Banerjee "Ankan Banerjee"), [CCC](/CCC "CCC"), June 22, 2013 » [GPU](/GPU "GPU"), [Kogge-Stone Algorithm](/Kogge-Stone_Algorithm "Kogge-Stone Algorithm") [[20]](#cite_note-20)
...
- [Auriga - distributed and collaborative Perft](http://www.talkchess.com/forum/viewtopic.php?t=58406) by [Giuseppe Cannella](/Giuseppe_Cannella "Giuseppe Cannella"), [CCC](/CCC "CCC"), November 28, 2015 [[21]](#cite_note-21)
...
- [Increase performance of make move, unmake move and move generation functions](https://talkchess.com/viewtopic.php?t=82843) by Elias Nilsson, [CCC](/CCC "CCC"), November 09, 2023
- [What is a good perft speed?](https://talkchess.com/viewtopic.php?t=83043) by Imno Umni, [CCC](/CCC "CCC"), December 20, 2023
- [Perft(16) estimate after averaging MC samples.](https://talkchess.com/viewtopic.php?t=83392) by Ajedrecista, [CCC](/CCC "CCC"), February 26, 2024

**2025...**

- [Are the perft results wrong on the CPW?](https://talkchess.com/viewtopic.php?t=84781) by [Tim Jones](/Tim_Jones "Tim Jones"), [CCC](/CCC "CCC"), January 30, 2025
- [The Grand Chess Tree](https://talkchess.com/viewtopic.php?t=84797) by [Tim Jones](/Tim_Jones "Tim Jones"), [CCC](/CCC "CCC"), February 04, 2025
- [Perft and theoretic draws (FIDE)](https://talkchess.com/viewtopic.php?t=84847) by [Yves De Billoëz](/Yves_De_Billo%C3%ABz "Yves De Billoëz"), [CCC](/CCC "CCC"), February 19, 2025
- [Perft engine - king moves](https://talkchess.com/viewtopic.php?t=84892) by Thomas Albert, [CCC](/CCC "CCC"), March 09, 2025
- [Perft stats discrepencies](https://talkchess.com/viewtopic.php?t=85136) by Ben Vining, [CCC](/CCC "CCC"), June 01, 2025
- [Writing the fastest move generator. Up to 4BNodes/s](https://talkchess.com/viewtopic.php?t=85453) by [Thomas Albert](/index.php?title=Thomas_Albert&action=edit&redlink=1 "Thomas Albert (page does not exist)"), [CCC](/CCC "CCC"), September 14, 2025
- [Why is my move generator so slow ?](https://talkchess.com/viewtopic.php?t=85560) by Paul Jérôme--Filio, [CCC](/CCC "CCC"), October 14, 2025
- [Get MoveGen errorfree](https://talkchess.com/viewtopic.php?t=85699) by Aldo Voogt, [CCC](/CCC "CCC"), November 23, 2025
...
- [The Grand Chess Tree](https://grandchesstree.com/) by [Tim Jones](/Tim_Jones "Tim Jones")
...
- [ankan-ban/perft\_gpu · GitHub](https://github.com/ankan-ban/perft_gpu) [[22]](#cite_note-22)
- [Auriga](http://cinnamonchess.altervista.org/auriga) by [Giuseppe Cannella](/Giuseppe_Cannella "Giuseppe Cannella") [[23]](#cite_note-23)
...
- [Crafty Command Documentation](http://www.craftychess.com/documentation/craftydoc.html) by [Robert Hyatt](/Robert_Hyatt "Robert Hyatt"), see perft <depth> » [Crafty](/Crafty "Crafty")
...
1. [↑](#cite_ref-1) [Re: Where to enter/read position into hash table in perft?](https://talkchess.com/viewtopic.php?p=836489) by [HGM's](/Harm_Geert_Muller "Harm Geert Muller"), [CCC](/CCC "CCC"), March 29, 2020
2. [↑](#cite_ref-2) [Written in Cobol - Program Written as Chess Buff's Research Aid](http://news.google.com/newspapers?nid=849&dat=19780417&id=h8lOAAAAIBAJ&sjid=DEoDAAAAIBAJ&pg=6180,1080528) by Brad Schultz, [Computerworld](/Computerworld "Computerworld"), April 17, 1978, Page 37
3. [↑](#cite_ref-3) [Perft(3) from 1978, with a twist!](http://www.talkchess.com/forum/viewtopic.php?t=41373) by [Steven Edwards](/Steven_Edwards "Steven Edwards"), [CCC](/CCC "CCC"), December 08, 2011
4. [↑](#cite_ref-4) [Re: Speed of Move Generator](https://groups.google.com/d/msg/rec.games.chess.computer/M8V1AzkfOok/YV9lcfOlfgIJ) by [Steven Edwards](/Steven_Edwards "Steven Edwards"), [rgcc](/Computer_Chess_Forums "Computer Chess Forums"), August 16, 1995
5. [↑](#cite_ref-5) [Re: complete opening tree stats](https://groups.google.com/d/msg/rec.games.chess.computer/2nqtCdHC-r0/ENqomE2u51kJ) by [Robert Hyatt](/Robert_Hyatt "Robert Hyatt"), [rgcc](/Computer_Chess_Forums "Computer Chess Forums"), February 05, 1998
6. [↑](#cite_ref-6) [Distributed perft project](https://www.stmintz.com/ccc/index.php?id=335026) by [Albert Bertilsson](/Albert_Bertilsson "Albert Bertilsson"), [CCC](/CCC "CCC"), December 09, 2003
7. [↑](#cite_ref-7) [Distributed Perft Project](https://web.archive.org/web/20061014115710/http://www.albert.nu/programs/dperft/) ([Wayback Machine](https://en.wikipedia.org/wiki/Wayback_Machine))
8. [↑](#cite_ref-8) [A048987](http://oeis.org/A048987) from [On-Line Encyclopedia of Integer Sequences](https://en.wikipedia.org/wiki/On-Line_Encyclopedia_of_Integer_Sequences) (OEIS)
9. [↑](#cite_ref-9) [Re: Perft(14) estimates thread](http://talkchess.com/forum/viewtopic.php?topic_view=threads&p=513308&t=47335) by [Peter Österlund](/Peter_%C3%96sterlund "Peter Österlund"), [CCC](/CCC "CCC"), April 02, 2013
10. [↑](#cite_ref-10) [MC methods](http://www.talkchess.com/forum/viewtopic.php?t=47740&start=2) by [Daniel Shawul](/Daniel_Shawul "Daniel Shawul"), [CCC](/CCC "CCC"), April 11, 2013
11. [↑](#cite_ref-11) [Daniel S. Abdi](/Daniel_Shawul "Daniel Shawul") (**2013**). *Monte carlo methods for estimating game tree size*. [pdf](https://dl.dropboxusercontent.com/u/55295461/perft/perft.pdf)
12. [↑](#cite_ref-12) [Re: yet another attempt on Perft(14)](http://www.talkchess.com/forum/viewtopic.php?t=61119&start=30) by [Ankan Banerjee](/Ankan_Banerjee "Ankan Banerjee"), [CCC](/CCC "CCC"), September 09, 2016
13. [↑](#cite_ref-13) [ankan-ban/perft\_gpu · GitHub](https://github.com/ankan-ban/perft_gpu)
14. [↑](#cite_ref-14) [DGX-1 for AI Research | NVIDIA](https://www.nvidia.com/en-us/data-center/dgx-1/)
15. [↑](#cite_ref-15) [Re: Perft(15): comparison of estimates with Ankan's result](http://www.talkchess.com/forum/viewtopic.php?t=64983&start=9) by [Ankan Banerjee](/Ankan_Banerjee "Ankan Banerjee"), [CCC](/CCC "CCC"), August 26, 2017
16. [↑](#cite_ref-16) [Re: perft(15)](http://www.talkchess.com/forum/viewtopic.php?t=64983&start=4) by [Ankan Banerjee](/Ankan_Banerjee "Ankan Banerjee"), [CCC](/CCC "CCC"), August 25, 2017
17. [↑](#cite_ref-17) [Re: Perft speed and depth questions](http://talkchess.com/forum3/viewtopic.php?f=7&t=74153) by [Mark Buisseret](/index.php?title=Mark_Buisseret&action=edit&redlink=1 "Mark Buisseret (page does not exist)"), [CCRL Discussion Board](/Computer_Chess_Forums "Computer Chess Forums"), June 12, 2020
18. [↑](#cite_ref-18) [Re: MC methods](http://www.talkchess.com/forum/viewtopic.php?t=47740&topic_view=flat&start=11) by [Daniel Shawul](/Daniel_Shawul "Daniel Shawul"), [CCC](/CCC "CCC"), April 13, 2013
19. [↑](#cite_ref-19) [Written in Cobol - Program Written as Chess Buff's Research Aid](http://news.google.com/newspapers?nid=849&dat=19780417&id=h8lOAAAAIBAJ&sjid=DEoDAAAAIBAJ&pg=6180,1080528) by Brad Schultz, [Computerworld](/Computerworld "Computerworld"), April 17, 1978, Page 37
20. [↑](#cite_ref-20) [ankan-ban/perft\_gpu · GitHub](https://github.com/ankan-ban/perft_gpu)
21. [↑](#cite_ref-21) [Auriga](http://cinnamonchess.altervista.org/auriga) by [Giuseppe Cannella](/Giuseppe_Cannella "Giuseppe Cannella")
22. [↑](#cite_ref-22) [Fast perft on GPU (upto 20 Billion nps w/o hashing)](http://www.talkchess.com/forum/viewtopic.php?t=48387) by [Ankan Banerjee](/Ankan_Banerjee "Ankan Banerjee"), [CCC](/CCC "CCC"), June 22, 2013
23. [↑](#cite_ref-23) [Auriga - distributed and collaborative Perft](http://www.talkchess.com/forum/viewtopic.php?t=58406) by [Giuseppe Cannella](/Giuseppe_Cannella "Giuseppe Cannella"), [CCC](/CCC "CCC"), November 28, 2015
