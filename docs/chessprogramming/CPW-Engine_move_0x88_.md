...
Source: https://www.chessprogramming.org/CPW-Engine_move(0x88)

**[Home](/Main_Page "Main Page") \* [Engines](/Engines "Engines") \* [CPW-Engine](/CPW-Engine "CPW-Engine") \* Move(0x88)**

Both move\_make and move\_unmake call the functions fillSq(color, piece, square) and clearSq(square). They are meant to encapsulate all the incremental updates(material and pcsq calues, hash keys etc.) and can be viewed in [CPW-Engine\_board(0x88)](/CPW-Engine_board(0x88) "CPW-Engine board(0x88)").Ideally this will make both make and unmake functions independent from the board representation. Beside that, incremental update code will be rather long, if not necessarily ugly.
