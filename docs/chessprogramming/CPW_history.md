...
Source: https://www.chessprogramming.org/CPW_history

**[Home](/Main_Page "Main Page") \* [Engines](/Engines "Engines") \* [CPW-Engine](/CPW-Engine "CPW-Engine") \* History**
...
- clearing of the search driver now done in search\_run, not in time\_xxx\_go
- more comments
...
- move\_isPseudoLegal() replaced by move\_isLegal() which tests for check

## 02.09.2008

- NEW: search\_iterate() stops on one legal reply after searching to depth 4
- new names and location of functions move\_isPseudoLegal() and move\_countLegal()
...
- serious restructuring of root search functions (sepatating search\_root and search\_iterate)
...
- FIX: in info\_currmove() first move number is 1, not 0 (bad C habits!)
- in search() variable move\_to\_make instead of a loop redetecting best move
...
- getting rid of is\_futility compiler switch
...
- added move\_iscapt() and move\_isprom() to improve readability
...
- separating com\_nothing() function
...
- stop tt\_save from overwriting an entry with the same position of a shallower depth
...
- making use of 088\_math.h and changing most of its functions into macros
- TWEAK: rooks on open files near the king increase tropism
- scaling of middlegame king evaluation affects also pcsq score
- creating a eval\_vect struct collecting partial scores
...
- separated PIECE\_VAL and SORT\_VAL, simplifying the code and getting rid of problems with king value
...
- FIX: solved the bug with the hash-values when time is over (tt\_save only saves the hash, when time\_over == false)
