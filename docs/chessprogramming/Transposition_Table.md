...
**[Home](/Main_Page "Main Page") \* [Search](/Search "Search") \* Transposition Table**

[![](/images/thumb/d/dd/The_Persistence_of_Memory.jpg/300px-The_Persistence_of_Memory.jpg)](https://en.wikipedia.org/wiki/The_Persistence_of_Memory)

[Salvador Dalí](/Category:Salvador_Dal%C3%AD "Category:Salvador Dalí"), [The Persistence of Memory](https://en.wikipedia.org/wiki/The_Persistence_of_Memory) 1931
...
The index is not based on the entire hash key because this is usually a 64-bit number, and with current hardware limitations, no hash table can be large enough to accommodate it. Therefore calculating the address or index requires a signature [modulo](https://en.wikipedia.org/wiki/Modulo_operator) a number of entries, for the power of two sized tables, the lower part of the hash key, masked by an 'and'-instruction accordantly.

# What Information is Stored

Typically, the following information is stored as determined by the [search](/Search "Search") [[6]](#cite_note-6) :

- [Zobrist-](/Zobrist_Hashing "Zobrist Hashing") or [BCH-key](/BCH_Hashing "BCH Hashing"), to see whether the position is the right one while probing
...
- [Type of Node](/Node_Types "Node Types") [[7]](#cite_note-7)
...
Depth, value, and bound information can be used to produce cutoffs and reduce redudant computation. In more advanced engines transposition table cutoffs are not performed on  [PV-Nodes](/Node_Types#PV-Nodes "Node Types").
...
- The entry type is  [EXACT](/Exact_Score "Exact Score")
- The entry type is  [LOWERBOUND](/Lower_Bound "Lower Bound") and greater than or equal to beta
- The entry type is  [UPPERBOUND](/Upper_Bound "Upper Bound") and less than alpha

## Hash Move

*Main Article: [Hash Move](/Hash_Move "Hash Move")*

Best move/Refutation move information from the transposition table is the most important part of move ordering. They are first in order and can be searched before other moves are generated. When the hash move fails high, generating other moves is not necessary. In [Stockfish](/Stockfish "Stockfish"), 75% of cutoffs produced in a position with hash moves are caused by the hash move.

## Singular Extensions

[Restricted SE](/Singular_Extensions#Restricted_SE "Singular Extensions") is only attempted on a position where a TT entry exist, they entry has a valid TT move, the entry has a high enough depth, and the entry has exact or lowerbound bounds.
...
| --- | --- |
| Upper bound for the number of reachable [chess positions](/Chess_Position "Chess Position") [[8]](#cite_note-8) | 1e46 |
| Different [64 bit](/Quad_Word "Quad Word") keys | 1.84e19 |
| Some number of distinct nodes searched per game, assuming 100 moves times 1e8 nodes per move | 1e10 |
| Different [32 bit](/Double_Word "Double Word") keys | 4.29e9 |
| Some arbitrary table size in the number of entries | 1e8 |

## Index Collisions

Index collisions or type-2 errors [[9]](#cite_note-9) [[10]](#cite_note-10), where different hash keys index same entries, happen regularly. They require detection, realized by storing the signature as part of the hash entry, to check whether a stored entry matches the position while probing. Especially with the power of two entry tables, many programmers choose to trade-off space for accuracy and only store that part of the hash key not already considered as an index, or even less.

## Key Collisions

Key collisions or type-1 errors are inherent in using signatures with far fewer bits than required to encode all reachable chess positions. A key collision occurs when two different positions map the same hash key or signature [[11]](#cite_note-11) [[12]](#cite_note-12). When storing only a partial key, the chance of a collision greatly increases. To accept only hits where [stored moves](/Hash_Move "Hash Move") are [pseudo-legal](/Pseudo-Legal_Move "Pseudo-Legal Move") decreases the chance of type-1 errors. On his computer chess page [[13]](#cite_note-13), [Ken Regan](/Kenneth_W._Regan "Kenneth W. Regan") broached on chess engine bugs, anomalies and hash collisions, and mentions a [Shredder 9.1](/Shredder "Shredder") game where a key collision might have caused a strange move [[14]](#cite_note-14) [[15]](#cite_note-15).

## Bits Required

During the [WCCC 1989](/WCCC_1989 "WCCC 1989") Workshop [New Directions in Game-Tree Search](/WCCC_1989#Workshop "WCCC 1989"), [James Gillogly](/James_Gillogly "James Gillogly"), author of [Tech](/Tech "Tech"), discussed transposition table collisions [[16]](#cite_note-16) . He produced the following table using the [Birthday Paradox](https://en.wikipedia.org/wiki/Birthday_problem), where the columns are the number of positions stored and the rows are the probability of collision. The entries are the number of bits of combined address and check hash required to reduce the probability of collision to the desired amount.
...
During the discussion, [David Slate](/David_Slate "David Slate") and [Ken Thompson](/Ken_Thompson "Ken Thompson") pointed out that the Birthday Paradox does not apply to most programs, since the hash table will fill up and not all previous positions will be in the table; thus these figures must be regarded as an upper bound on the number of bits required for safety [[17]](#cite_note-17). The dangers of transposition table collisions were further studied by [Robert Hyatt](/Robert_Hyatt "Robert Hyatt") and [Anthony Cozzie](/Anthony_Cozzie "Anthony Cozzie") as published in their 2005 paper *Hash Collisions Effect* [[18]](#cite_note-18). They gave a surprising answer to the question “Is it really worth all the effort to absolutely minimize signature collisions?”, and concluded that 64-bit signatures are more than sufficient.
...
This replacement strategy uses the position of entry move in [move ordering](/Move_Ordering "Move Ordering") list as replacement priority. The main idea is that if the best move was not considered as a good cut-off candidate by the move-ordering algorithm, storing it in TT should provide better help for the search.
...
Aging considers searches for different chess positions during gameplay. While early implementations and programs relying on [root pre-processing](/Piece-Square_Tables#Preprocessing "Piece-Square Tables") to guide search and [evaluation](/Evaluation "Evaluation") were obligated to clear the hash table between root positions, most todays programs do not, profit from entries of previous searches. Nevertheless, to avoid persistence of old entries which may no longer occur from the current root, aging is used to likely replace those entries by new ones, even if their draft and flags would otherwise protect them. To implement aging, one often stores and compares the current [halfmove clock](/Halfmove_Clock "Halfmove Clock") as age, likely modulo some power two constant, depending on how many bits are used to store it inside an entry [[24]](#cite_note-24) [[25]](#cite_note-25).
...
- [Checking TT move for legality](https://talkchess.com/viewtopic.php?t=82494) by Aaron Li, [CCC](/CCC "CCC"), August 20, 2023

**2024**

- [Help with transposition table](https://talkchess.com/viewtopic.php?t=83222) by E Boatwright, [CCC](/CCC "CCC"), January 21, 2024
- [adding TT reduces NPS by allot](https://talkchess.com/viewtopic.php?t=83549) by Ravael Jansen, [CCC](/CCC "CCC"), March 29, 2024
- [Hybride replacemment strategy worse than always-replace](https://talkchess.com/viewtopic.php?t=83695) by mathmoi, [CCC](/CCC "CCC"), April 25, 2024
...
6. [↑](#cite_ref-6) [Dennis Breuker](/Dennis_Breuker "Dennis Breuker"), [Jos Uiterwijk](/Jos_Uiterwijk "Jos Uiterwijk"), [Jaap van den Herik](/Jaap_van_den_Herik "Jaap van den Herik") (**1997**). *Information in Transposition Tables*. [Advances in Computer Chess 8](/Advances_in_Computer_Chess_8 "Advances in Computer Chess 8")
7. [↑](#cite_ref-7) [MTD(f)](/MTD(f) "MTD(f)")- and some [PVS](/Principal_Variation_Search "Principal Variation Search")-implementations store distinct [upper](/Upper_Bound "Upper Bound") and [lower bound](/Upper_Bound "Upper Bound") scores, rather than one score with flags
8. [↑](#cite_ref-8) [Shirish Chinchalkar](/Shirish_Chinchalkar "Shirish Chinchalkar") (**1996**). *An Upper Bound for the Number of Reachable Positions*. [ICCA Journal, Vol. 19, No. 3](/ICGA_Journal#19_3 "ICGA Journal")
9. [↑](#cite_ref-9) [Albert Zobrist](/Albert_Zobrist "Albert Zobrist") (**1970**). *A New Hashing Method with Application for Game Playing*. Technical Report #88, Computer Science Department, The University of Wisconsin, Madison, WI, USA. Reprinted (1990) in [ICCA Journal, Vol. 13, No. 2](/ICGA_Journal#13_2 "ICGA Journal"), [pdf](http://www.cs.wisc.edu/techreports/1970/TR88.pdf)
10. [↑](#cite_ref-10) [Collision probability](http://groups.google.com/group/rec.games.chess.computer/browse_frm/thread/b9de346eb1e8300/) by [Dennis Breuker](/Dennis_Breuker "Dennis Breuker"), [rgcc](/Computer_Chess_Forums "Computer Chess Forums"), April 15, 1996
11. [↑](#cite_ref-11) [TT Key Collisions, Workarounds?](http://www.talkchess.com/forum/viewtopic.php?t=40062) by [Clemens Pruell](/index.php?title=Clemens_Pruell&action=edit&redlink=1 "Clemens Pruell (page does not exist)"), [CCC](/CCC "CCC"), August 16, 2011
12. [↑](#cite_ref-12) [how to measure frequency of hash collisions](http://www.talkchess.com/forum/viewtopic.php?t=44082) by [Daniel Shawul](/Daniel_Shawul "Daniel Shawul"), [CCC](/CCC "CCC"), June 16, 2012
13. [↑](#cite_ref-13) [Computer Chess - Hash Collisions in Chess Engines, and What They May Mean...](http://www.cse.buffalo.edu/~regan/chess/computer/) by [Kenneth W. Regan](/Kenneth_W._Regan "Kenneth W. Regan")
14. [↑](#cite_ref-14) [Pablo Lafuente vs Shredder (Computer) (2005)](http://www.chessgames.com/perl/chessgame?gid=1352348) from [chessgames.com](http://www.chessgames.com/index.html)
15. [↑](#cite_ref-15) [Current tournaments – Sanjin, Biel, Argentina, Israel](http://www.chessbase.com/newsdetail.asp?newsid=2525), [ChessBase News](/ChessBase "ChessBase"), July 21, 2005
16. [↑](#cite_ref-16) [James Gillogly](/James_Gillogly "James Gillogly") (**1989**). *Transposition Table Collisions*. [Workshop on New Directions in Game-Tree Search](/WCCC_1989#Workshop "WCCC 1989")
...
18. [↑](#cite_ref-18) [Robert Hyatt](/Robert_Hyatt "Robert Hyatt"), [Anthony Cozzie](/Anthony_Cozzie "Anthony Cozzie") (**2005**). *[The Effect of Hash Signature Collisions in a Chess Program](http://www.craftychess.com/hyatt/collisions.html)*. [ICGA Journal, Vol. 28, No. 3](/ICGA_Journal#28_3 "ICGA Journal")
