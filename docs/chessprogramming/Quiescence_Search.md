...
**[Home](/Main_Page "Main Page") \* [Search](/Search "Search") \* Quiescence**

[![](/images/f/ff/Quiescence.jpg)](http://www.yogachicago.com/jan06/art.shtml)

[Karen Schuman](/index.php?title=Category:Karen_Schuman&action=edit&redlink=1 "Category:Karen Schuman (page does not exist)") - Quiescence [[1]](#cite_note-1)
...
Since quiescence search has no depth limit, it's vulnerable to [search explosions](/Search_Explosion "Search Explosion") in the absence of any [move ordering](/Move_Ordering "Move Ordering"). For this reason, it's crucial to introduce simple capture ordering such as [MVV-LVA](/MVV-LVA "MVV-LVA") before implementing qsearch. Although individual quiescence searches are typically shallow, they can consume a significant percentage of total nodes, so it is also worthwhile to apply some [pruning](/Pruning "Pruning"). Apart from not trying moves with the [static exchange evaluation](/Static_Exchange_Evaluation "Static Exchange Evaluation") < 0, [delta pruning](/Delta_Pruning "Delta Pruning") can be used for that purpose.
