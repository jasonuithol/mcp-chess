...
**[Home](/Main_Page "Main Page") \* [Search](/Search "Search") \* [Selectivity](/Selectivity "Selectivity") \* [Pruning](/Pruning "Pruning") \* Null Move Pruning**

[![](/images/thumb/f/f0/SamuelBakSignals.jpg/300px-SamuelBakSignals.jpg)](http://chgs.elevator.umn.edu/asset/viewAsset/57f3b6787d58ae5f74bf8ba9#57f3b6d77d58ae5574bf8bc5)

[Samuel Bak](/Category:Samuel_Bak "Category:Samuel Bak") - Signals [[1]](#cite_note-1)
...
**Null Move Observation**: In almost all chess positions, making a null move (passing a turn) is worse than the best legal move.

NMP takes advantage of this observation, and it works as follows. If a reduced search on a null move fails high over beta, then by the null move observation, we can be quite confident that the best legal move would also fail high over beta. This means that once a null move search fails high, we can return a fail high at that point, skipping later move generation or any full depth searches.

One particular exception to the Null Move Observation is the case zugzwangs. When every possible move from the player will worsen their position. This can be avoided by disabling NMP at pawn endgames, where zugzwangs are prone. For piece endgames, verification search can be an effective heuristic in preventing zugzwang blindness from NMP.
...
A basic implementation of NMP only guards against positions in check and positions where the side to move has only king and pawns. This is because zugzwangs are particularly prone in positions without pieces, and may have a significant role in determining who wins the ensuing endgame. For correctness reasons, some implementations also disables consecutive null moves, although they may not have an effect on engine strength.

## Reduction

A basic implementation of NMP uses a fixed reduction of 3 or 4. The reduced null move search is searched at null window [-beta, -beta + 1].

# Advanced Tweaks

The following are common guards to restrict the use of NMP in some situations. They should be tested incrementally:

- Inside a non-[PV](/Node_Types#PV-Nodes "Node Types") node.
- Inside a [cut node](/Node_Types#Cut-Nodes "Node Types").
- When (potentially TT-corrected) evaluation is higher or equal to beta.
- When static evaluation is higher than beta over a margin.

An advanced implementation of NMP also adjusts reduction value by certain parameters. For example, a depth scaling factor, such as depth / 3, can be added to the depth reduction. Additionally, a factor scaled by the difference between (potentially TT-corrected) evaluation and beta can also be added to the reduction value. Optimizing NMP parameters can bring significant Elo to a program.
